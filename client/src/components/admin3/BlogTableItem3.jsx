...
const BlogTableItem3 = ({blog,fetchBlogs,index}) => {
    ...
    const deleteBlog = async ()=>{
      ...
      if(!confirm) return;
      try {
        const {data} = await axios.post('/api/blog/delete3',{id: blog._id})
        if(data.success){
    ...
    const togglePublish = async ()=>{
      try {
        const {data} = await axios.post('/api/blog/toggle-publish3',{id: blog._id})
        if(data.success){
          toast.success(data.message)
          await fetchBlogs()
        }else{
