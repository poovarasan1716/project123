...
const [category,setCategory]= useState('AI Club');
...
const generateContent = async ()=>{
     if(!title) return toast.error('Please enter a title')
      ...
      try {
        setLoading(true);
        const {data} = await axios.post('/api/blog/generate2',{prompt: title})
        if (data.success){
...
const onSubmitHandled = async(e) =>{
 try {
   ...
   const {data} = await axios.post('/api/blog/add2', formData);
...
useEffect(()=>{
 if(!quillRef.current && editorRef.current){
  ...
  return (
    ...
    <form onSubmit={onSubmitHandled} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
